...
<div class="container-fluid py-4">
    <div class="row">
        <div class="col-md-3">
            <a class="nav-link text-white border-radius-lg text-center me-2 mb-5 bg-gradient-success" href="{{ route('admin.create.entre') }}">
              ...
              <h3>Gestion d'entré</h3>
                <div class="cards2 w-100">
                  <form action="" class="cards">
                    ...
                    <input type="text" class="form-control" name="inputSearchentre" id="inputSearchentre" value="{{  request()->input('inputSearchentre')}}">
                    ...
                    @if (Request::query('inputSearchentre') || Request::query('dateDebut') || Request::query('dateFin'))
                      <a href="{{ route('admin.entre') }}" class="btn btn-warning font-weight-bold text-xs badge h-100 text-center"> Annuler </a>
                    @endif
                    <form action="{{ route('entre.exportPDF') }}" class="d-flex">
                      ...
                      <input type="hidden" class="form-control" name="inputSearchentre" id="inputSearchentre" value="{{  request()->input('inputSearchentre')}}">
                      <button type="submit" class="btn btn-danger font-weight-bold text-xs badge h-100 w-100 text-center" >PDF</button>
                    </form>
                    <form action="{{ route('entre.exportArchive') }}" class="d-flex">
                      <input type="hidden" class="form-control" name="dateDebut" id="dateDebut" value="{{ request()->input('dateDebut')}}">
                      <input type="hidden" class="form-control" name="dateFin" id="dateFin" value="{{ request()->input('dateFin')}}">
                      <input type="hidden" class="form-control" name="inputSearcharchive" id="inputSearcharchive" value="{{  request()->input('inputSearchentre')}}">
                    ...
                    @foreach ($entres as $entre)
                        <tr>
                            ...
                            <td class="align-middle d-grid">
                              <form action="{{ route('admin.delete.entre',$entre->id) }}" method="POST" id="from_delete" class="w-100">
                                  @csrf
                                ...
                                <a href="{{ route('admin.edit.entre',$entre->id) }}" class="btn btn-info font-weight-bold text-xs badge " data-toggle="tooltip" data-original-title="Edit user">
                                  modifier
                                </a>
                                <form action="{{ route('admin.valide.entre',$entre->id) }}" method="POST" id="from_delete">
                                    @csrf
                                    <input type="hidden" name='validation' value="{{ $entre->validation }}">
...
@section('javascript_entre')
  <script>
    ...
    function sort_search()
    {
      ...
      window.location.href= "{{ route('admin.entre') }}?"+$.param(query);
    }
