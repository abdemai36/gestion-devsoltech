                ...
                <div class="col-md-12 d-flex flex-column ">
                  <div class="card card-plain">
                    <div class="card-header">
                      <h4 class="font-weight-bolder">Modifier L'entré</h4>
                      <p class="mb-0"></p>
                    </div>
                    <div class="card-body" style="background: white;border-radius: 0 0 0.75rem 0.75rem">
                      ...
                      @endif
                      <form role="form" method="POST" action="{{ route('user.entre.update',$entre->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <input type="hidden" name="idUser" value="{{ Auth::user()->id }}">
                        <label class="form-label">Date d'entre</label>
                        <div class="input-group input-group-outline mb-3">
                          <input type="text" name="date" class="form-control" value="{{ $entre->date }}" readonly>
                        </div>
                        <label class="form-label">Nom</label>
                        <div class="input-group input-group-outline mb-3">
                          <input type="text" name="name" class="form-control" value="{{ $entre->name }}" readonly>
                        </div>
                        ...
                        <div class="input-group input-group-outline mb-3">
                          ...
                          <input type="number" name="montant" class="form-control" value="{{ $entre->montant }}">
                        </div>
                        ...
                        <div class="form-check form-check-info text-start ps-0">
                            ...
                            <textarea style="border: 1px solid #ddd" class="form-control" name="designiation" id="Désignation">{{ $entre->designiation }}</textarea> 
                        </div>
